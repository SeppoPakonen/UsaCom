#!/usr/bin/env python3
"""Process all remaining Phase 4 tasks (pending files)."""
...
def get_already_processed():
    ...
    for f in list(processed_dir.glob("*_parsed.json")) + list(processed_dir.glob("*_processed.json")):
        name = f.stem.replace("_parsed", "").replace("_processed", "")
...
def main():
    ...
    # Get all export files (excluding _fixed)
    export_files = sorted([f for f in parser.export_dir.glob("*.md") if not f.name.endswith("_fixed.md")])
    
    # Get already processed files
    already_processed = get_already_processed()
    
    # Filter to only unprocessed files
    files_to_process = [f for f in export_files if f.stem not in already_processed]
    
    print(f"Total export files: {len(export_files)}")
    ...
    # Process each file
    for i, input_file in enumerate(files_to_process, start=1):
        output_file = parser.processed_dir / f"{input_file.stem}_parsed.json"
